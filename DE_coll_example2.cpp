...
{
  return sqrt(1 + x*x) * sinh(atan(x));
}

double ex2_u(double x)
{
  return (cosh(atan(x)) + x*sinh(atan(x)))/sqrt(1 + x*x);
}
...
int main()
{
  ...
  for (int i = -N; i <= N; i++) {
    for (int j = -N; j <= N; j++) {
      ...
      A_N(i+N+m, j+N)   = - 2 * h * DE_trans_div(a, b, j*h) * del1(i, j) / ((1+tmp*tmp)*(1+tmp*tmp));
  ...
  for (int i=1; i< SAMPLE; i++) {
    ...
    for (int j=N; j>0; j--) {
      ans += (y_1( j+N) - y_1(-N+N)*waDE( j*h) - y_1( N+N)*wbDE( j*h))
                       * S( j, h, DE_trans_inv(a, b, x));
      ans += (y_1(-j+N) - y_1(-N+N)*waDE(-j*h) - y_1( N+N)*wbDE(-j*h))
                       * S(-j, h, DE_trans_inv(a, b, x));
    }
      ans += (y_1( 0+N) - y_1(-N+N)*waDE(0*h) - y_1( N+N)*wbDE(0*h))
                       * S( 0, h, DE_trans_inv(a, b, x));
  ...
  for (int i=1; i< SAMPLE; i++) {
    ...
    for (int j=N; j>0; j--) {
      ans += (y_2( j+N) - y_2(-N+N)*waDE( j*h) - y_2( N+N)*wbDE( j*h))
                       * S( j, h, DE_trans_inv(a, b, x));
      ans += (y_2(-j+N) - y_2(-N+N)*waDE(-j*h) - y_2( N+N)*wbDE(-j*h))
                       * S(-j, h, DE_trans_inv(a, b, x));
    }
      ans += (y_2( 0+N) - y_2(-N+N)*waDE(0*h) - y_2( N+N)*wbDE(0*h))
                       * S( 0, h, DE_trans_inv(a, b, x));
