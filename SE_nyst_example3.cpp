...
int main()
{
  ...
  for (int i=1; i< SAMPLE; i++) {
    ...
    for (int j=N; j>=-N; j--) {
      ans += (0.5 * y_2( j+N)/sqrt(SE_trans(a, b, j*h)) - y_1(j+N))
            * SE_trans_div(a, b, j*h) * J( j, h, SE_trans_inv(a, b, x));
    }
    ans += 0;
  ...
  for (int i=1; i< SAMPLE; i++) {
    ...
    for (int j=N; j>= -N; j--) {
      ans += - y_1( j+N) * SE_trans_div(a, b, j*h)
             * J( j, h, SE_trans_inv(a, b, x))/sqrt(SE_trans(a, b, j*h));
    }
    ans += 1;
