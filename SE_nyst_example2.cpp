...
int main()
{
  ...
  for (int i = -N; i <= N; i++) {
    for (int j = -N; j <= N; j++) {
      ...
      A_N(i+N+m, j+N)   = - 2 * h * SE_trans_div(a, b, j*h) * del1(i, j) / ((1+tmp*tmp)*(1+tmp*tmp));
      A_N(i+N  , j+N+m) = - h * SE_trans_div(a, b, j*h) * del1(i, j);
    }
  }
  ...
  for (int i=1; i< SAMPLE; i++) {
    ...
    for (int j=N; j>0; j--) {
      ...
      ans += 2 * y_1( j+N) * SE_trans_div(a, b, j*h)
                       * J( j, h, SE_trans_inv(a, b, x)) / ((1+tmp*tmp)*(1+tmp*tmp));
      tmp = SE_trans(a, b,-j*h);
      ans += 2 * y_1(-j+N) * SE_trans_div(a, b,-j*h)
                       * J(-j, h, SE_trans_inv(a, b, x)) / ((1+tmp*tmp)*(1+tmp*tmp));
    }
      ...
      ans += 2 * y_1( 0+N) * SE_trans_div(a, b, 0*h)
                       * J( 0, h, SE_trans_inv(a, b, x)) / ((1+tmp*tmp)*(1+tmp*tmp));
    ans += 1;
